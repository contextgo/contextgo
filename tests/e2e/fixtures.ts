 ...
 *      (e.g. out/linux-unpacked/contextgo, out/mac-arm64/ContextGo.app, out/win-unpacked/ContextGo.exe).
...
const e2eStateSandboxDir = fs.mkdtempSync(path.join(os.tmpdir(), 'contextgo-e2e-state-'));
...
function resolvePackagedApp(): { executablePath: string; cwd: string } | null {
  ...
  if (platform === 'win32') {
    // out/win-unpacked/ContextGo.exe  or  out/win-x64-unpacked/ContextGo.exe
    for (const dir of ['win-unpacked', 'win-x64-unpacked', 'win-arm64-unpacked']) {
      const exe = path.join(outDir, dir, 'ContextGo.exe');
      if (fs.existsSync(exe)) return { executablePath: exe, cwd: path.join(outDir, dir) };
    }
  } else if (platform === 'darwin') {
    // out/mac-arm64/ContextGo.app/Contents/MacOS/ContextGo  or  out/mac/ContextGo.app/...
    for (const dir of ['mac-arm64', 'mac-x64', 'mac', 'mac-universal']) {
      ...
      if (appBundle) {
        const exe = path.join(macDir, appBundle, 'Contents', 'MacOS', 'ContextGo');
        if (fs.existsSync(exe)) return { executablePath: exe, cwd: macDir };
      }
    }
  } else {
    // Linux: out/linux-unpacked/contextgo  (lowercase executable name)
    for (const dir of ['linux-unpacked', 'linux-x64-unpacked', 'linux-arm64-unpacked']) {
      const dirPath = path.join(outDir, dir);
      if (!fs.existsSync(dirPath)) continue;
      // Try common executable names
      for (const name of ['contextgo', 'ContextGo']) {
...
async function launchApp(): Promise<ElectronApplication> {
  ...
  const commonEnv = {
    ...process.env,
    CONTEXTGO_EXTENSIONS_PATH: process.env.CONTEXTGO_EXTENSIONS_PATH || path.join(projectRoot, 'examples'),
    CONTEXTGO_EXTENSION_STATES_FILE: process.env.CONTEXTGO_EXTENSION_STATES_FILE || e2eStateFile,
    CONTEXTGO_DISABLE_AUTO_UPDATE: '1',
    CONTEXTGO_DISABLE_DEVTOOLS: '1',
    CONTEXTGO_E2E_TEST: '1',
    CONTEXTGO_CDP_PORT: '0',
  };
