...
const emptyExtensionsDir = fs.mkdtempSync(path.join(os.tmpdir(), 'contextgo-e2e-no-extensions-'));
const stateSandboxDir = fs.mkdtempSync(path.join(os.tmpdir(), 'contextgo-e2e-no-extensions-state-'));
...
async function launchAppWithoutExtensions(): Promise<ElectronApplication> {
  ...
  return electron.launch({
    ...
    env: {
      ...process.env,
      CONTEXTGO_EXTENSIONS_PATH: emptyExtensionsDir,
      CONTEXTGO_EXTENSION_STATES_FILE: extensionStatesFile,
...
test.describe.serial('Extension: Empty Directory / No Extensions', () => {
  ...
  test('loads with zero extension contributions', async () => {
    const [loadedExtensions, acpAdapters, assistants, agents, skills, themes, settingsTabs, webuiContributions] =
      await Promise.all([
