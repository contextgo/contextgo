...
describe('ExternalSessionDiscoveryService', () => {
  ...
  it('lists unmanaged OpenClaw sessions from the local gateway', async () => {
    const service = new ExternalSessionDiscoveryService(createConversationService([]), {
      availableBackends: new Set<AcpBackendAll>(['openclaw-gateway']),
    });

    vi.spyOn(service as never, 'listOpenClawAgents' as never).mockReturnValue([
      {
        agentId: 'main',
        name: 'OpenClaw',
        ...
        sessions: [
          {
            ...
            derivedTitle: 'ignore me',
            updatedAt: 1_775_100_000_000,
            origin: {
              provider: 'heartbeat',
            },
          },
          {
            ...
            agentId: 'main',
            displayName: 'OpenClaw Control',
            derivedTitle: 'Investigate import flow',
            updatedAt: 1_775_100_100_000,
            modelProvider: 'infermesh',
        ...
        sessions: [
          {
            ...
            agentId: 'dev',
            displayName: 'Dev',
            lastMessagePreview: 'Fix the flaky test suite',
            updatedAt: 1_775_100_200_000,
            modelProvider: 'infermesh',
            model: 'MiniMax-M2.7-highspeed',
            origin: {
              provider: 'webchat',
    ...
    vi.spyOn(service as never, 'withOpenClawGatewayConnection' as never).mockImplementation(async (callback) =>
      callback({
        sessionsList,
  ...
  it('lists heartbeat-labeled OpenClaw sessions when history shows real control-ui chat', async () => {
    const service = new ExternalSessionDiscoveryService(createConversationService([]), {
      availableBackends: new Set<AcpBackendAll>(['openclaw-gateway']),
    });
          ...
          sessions: [
            {
              ...
              agentId: 'main',
              displayName: 'heartbeat',
              lastMessagePreview: '你是啥模型',
              updatedAt: 1_775_100_300_000,
              modelProvider: 'infermesh',
              model: 'MiniMax-M2.7-highspeed',
              origin: {
                provider: 'webchat',
  ...
  it('uses the session agent metadata for OpenClaw workspace and grouping', async () => {
    const service = new ExternalSessionDiscoveryService(createConversationService([]), {
      availableBackends: new Set<AcpBackendAll>(['openclaw-gateway']),
    });
        ...
        sessions: [
          {
            ...
            agentId: 'main',
            displayName: 'Shared Main Session',
            updatedAt: 1_775_100_400_000,
            origin: {
              provider: 'webchat',
              surface: 'webchat',
            },
    ...
    expect(sessions).toEqual([
      {
        provider: 'openclaw-gateway',
        sessionId: 'agent:main:shared',
        title: 'Shared Main Session',
        workspace: '/tmp/openclaw-main',
        updatedAt: 1_775_100_400_000,
        origin: 'webchat',
  ...
  it('normalizes OpenClaw agent ids before mapping sessions to configured agents', async () => {
    const service = new ExternalSessionDiscoveryService(createConversationService([]), {
      availableBackends: new Set<AcpBackendAll>(['openclaw-gateway']),
    });
    ...
    const sessionsList = vi
      ...
      .mockResolvedValueOnce({
        sessions: [
          {
            key: 'agent:main:mixed-case',
            agentId: 'DEV',
            displayName: 'Dev Session',
            updatedAt: 1_775_100_500_000,
            origin: {
              provider: 'webchat',
              surface: 'webchat',
            },
    ...
    expect(sessions).toEqual([
      {
        provider: 'openclaw-gateway',
        sessionId: 'agent:main:mixed-case',
        title: 'Dev Session',
        workspace: '/tmp/openclaw-dev',
        updatedAt: 1_775_100_500_000,
        origin: 'webchat',
  ...
  it('creates OpenClaw conversations when taking over an external OpenClaw session', async () => {
    const service = new ExternalSessionDiscoveryService(createConversationService([]), {
      availableBackends: new Set<AcpBackendAll>(['openclaw-gateway']),
    });
  ...
  it('imports visible OpenClaw history when taking over an unmanaged session', async () => {
    const insertMessage = vi.fn(() => ({
      success: true,
    }));
    vi.spyOn(databaseModule, 'getDatabase').mockResolvedValue({
          ...
          messages: [
            ...
            },
            {
              ...
              content: [
                {
                  type: 'text',
                  text: 'ignore tool result',
                },
              ],
              timestamp: 1_775_100_002_000,
            },
            {
              role: 'user',
              content: [
