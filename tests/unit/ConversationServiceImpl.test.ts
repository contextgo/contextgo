...
describe('ConversationServiceImpl.getConversation', () => {
  ...
  it('attaches the default space when legacy conversation extras have no spaceId', async () => {
    const repo = makeRepo({
    ...
    const result = await svc.getConversation('c1');

    expect(result?.extra.spaceId).toBe('space-default');
    expect(result?.extra.workingDirectory).toBe('/ws/project');
  });
...
describe('ConversationServiceImpl.createWithMigration', () => {
  ...
  it('copies messages from source conversation', async () => {
    ...
    const repo = makeRepo({
      getMessages: vi
        .fn()
        .mockReturnValueOnce({ data: [msg], total: 1, hasMore: false })
        .mockReturnValue({ data: [], total: 1, hasMore: false }),
    });
    const svc = new ConversationServiceImpl(repo, makeSpaceService());
