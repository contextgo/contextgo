...
describe('PublicationBindingPanel', () => {
  ...
  it('shows channel account entries on the left and switches the management console with the selected account', async () => {
    ...
    await screen.findByText('Publication setup');
    ...
    expect(screen.getAllByText('Ready to publish').length).toBeGreaterThanOrEqual(1);
    expect(screen.getAllByText('Published: 0').length).toBeGreaterThanOrEqual(2);
    expect(screen.getByText('Currently only using fallback runtime')).toBeInTheDocument();
    ...
    expect(screen.queryByText('Currently only using fallback runtime')).not.toBeInTheDocument();
  });
  ...
  it('keeps discovered targets readable instead of exposing routing internals', async () => {
    ...
    expect(screen.getByText('Target detail:')).toBeInTheDocument();
    expect(screen.getAllByText('Topic root 1').length).toBeGreaterThanOrEqual(1);
    expect(screen.getByText('Session behavior:')).toBeInTheDocument();
    expect(screen.getByText('Each topic is isolated as its own long-term session.')).toBeInTheDocument();
    expect(screen.queryByText('Peer key:')).not.toBeInTheDocument();
    expect(screen.queryByText('Parent target:')).not.toBeInTheDocument();
    expect(screen.getAllByText('Published: 0').length).toBeGreaterThanOrEqual(2);
    expect(screen.getAllByText('Discovered targets: 1').length).toBeGreaterThanOrEqual(2);
  });
