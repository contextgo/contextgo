...
class CloudApiTestCase(unittest.TestCase):
    ...
    def test_infermesh_provider_uses_browser_session(self) -> None:
        ...
        provision.assert_awaited_once_with(self.settings, user, group=None)

    def test_infermesh_token_groups_accept_device_token(self) -> None:
        registration = self._register_device(device_name="Studio", platform="macos")
        device_token = registration["token"]
        self.client.cookies.pop(self.settings.session_cookie_name, None)
        groups = [
        ...
        with patch.object(
            ...
            response = self.client.get(
                "/api/integrations/infermesh/token-groups",
                headers={"Authorization": f"Bearer {device_token}"},
            )

        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json()["groups"], groups)
        list_groups.assert_awaited_once_with(self.settings)
    ...
    def test_infermesh_provider_accepts_device_token(self) -> None:
        ...
        with patch.object(
            ...
            response = self.client.get(
                "/api/integrations/infermesh/provider?group=claude-0.5x",
                headers={"Authorization": f"Bearer {device_token}"},
            )
        ...
        self.assertEqual(provision.await_args.kwargs["group"], "claude-0.5x")
