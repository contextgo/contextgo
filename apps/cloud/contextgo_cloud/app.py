...
@app.get("/api/integrations/infermesh/token-groups")
async def infermesh_token_groups(request: Request) -> JSONResponse:
    ...
    require_current_user_or_device(request)
    try:
        groups = await list_infermesh_token_groups(settings)
    except InfermeshProvisionError as error:
        raise HTTPException(
            status_code=status.HTTP_502_BAD_GATEWAY,
            detail=str(error),
...
async def infermesh_provider(request: Request, group: Optional[str] = Query(default=None)) -> JSONResponse:
    if not is_infermesh_configured(settings):
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="InferMesh integration is not configured",
    ...
    user = require_current_user_or_device(request)
    try:
        provider = await provision_infermesh_provider(settings, user, group=group)
    except InfermeshProvisionError as error:
        raise HTTPException(
            status_code=status.HTTP_502_BAD_GATEWAY,
            detail=str(error),
