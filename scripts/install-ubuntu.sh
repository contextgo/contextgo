#!/usr/bin/env bash
# ============================================================================
# ContextGo — Ubuntu / Debian 一鍵自動化安裝腳本
# ============================================================================
# 功能：
...
#   5. 建立服務管理腳本 (/opt/ContextGo/start-contextgo.sh)
#   6. (可選) 建立 systemd service
#   7. (可選) 建立桌面捷徑
#
# 用法：
#   curl -fsSL https://raw.githubusercontent.com/contextgo/contextgo/main/scripts/install-ubuntu.sh | bash
#   # 或指定版本：
#   CONTEXTGO_VERSION=1.8.25 bash install-ubuntu.sh
#   # 僅安裝桌面版（跳過 headless 設定）：
#   CONTEXTGO_MODE=desktop bash install-ubuntu.sh
# ============================================================================
...
banner() {
    echo -e "${CYAN}${BOLD}"
    echo "  ╔══════════════════════════════════════════════╗"
    echo "  ║         ContextGo Installer for Ubuntu       ║"
    echo "  ╚══════════════════════════════════════════════╝"
    echo -e "${NC}"
}
...
resolve_version() {
    ...
    if [[ -n "$requested_version" ]]; then
        ...
        if [[ -z "$VERSION" ]]; then
            die "無法取得最新版本號，請手動指定: CONTEXTGO_VERSION=1.8.25 bash $0"
        fi
...
install_deb() {
    ...
    success "ContextGo v${VERSION} 安裝完成"

    # 驗證安裝
    if command -v ContextGo &>/dev/null || [[ -x /usr/bin/ContextGo ]]; then
        success "ContextGo 已安裝至 $(which ContextGo 2>/dev/null || echo '/usr/bin/ContextGo')"
    else
        warn "安裝可能不完整，找不到 ContextGo 執行檔"
    fi
...
create_service_script() {
    local script_dir="/opt/ContextGo"
    local script_path="${script_dir}/start-contextgo.sh"
...
#!/bin/bash
# ============================================================================
# ContextGo WebUI Headless 服務管理腳本
# 用法: ./start-contextgo.sh [start|stop|restart|status|logs]
# ============================================================================
...
start() {
    if [ -f "$PIDFILE" ] && kill -0 "$(cat "$PIDFILE")" 2>/dev/null; then
        echo "⚡ ContextGo 已在執行中 (PID: $(cat "$PIDFILE"))"
        return 1
    fi

    echo "🚀 正在啟動 ContextGo WebUI..."
    cd "$WORKDIR" || exit 1
    ...
    if kill -0 "$(cat "$PIDFILE")" 2>/dev/null; then
        echo "✅ ContextGo 啟動成功 (PID: $(cat "$PIDFILE"))"
        local ip
        ip=$(hostname -I 2>/dev/null | awk '{print $1}')
        echo "🌐 WebUI: http://${ip:-localhost}:25808"
    else
        echo "❌ ContextGo 啟動失敗，請查看日誌: $LOGFILE"
        rm -f "$PIDFILE"
        return 1
    fi
}
...
status() {
    if [ -f "$PIDFILE" ] && kill -0 "$(cat "$PIDFILE")" 2>/dev/null; then
        echo "✅ ContextGo 執行中 (PID: $(cat "$PIDFILE"))"
...
create_systemd_service() {
    ...
    local service_path="/etc/systemd/system/contextgo.service"

    info "建立 systemd 服務: $service_path"

    $SUDO tee "$service_path" > /dev/null << 'SERVICE_EOF'
[Unit]
Description=ContextGo AI Agent Desktop App (WebUI Mode)
Documentation=https://github.com/contextgo/contextgo
...
ExecStart=/usr/bin/xvfb-run --auto-servernum --server-args="-screen 0 1920x1080x24" /usr/bin/ContextGo --webui --remote --no-sandbox
Restart=on-failure
RestartSec=10
...
SERVICE_EOF
    ...
    info "使用方式:"
    echo "    sudo systemctl start contextgo     # 啟動"
    echo "    sudo systemctl stop contextgo      # 停止"
    echo "    sudo systemctl enable contextgo    # 開機自動啟動"
    echo "    sudo systemctl status contextgo    # 查看狀態"
    echo "    journalctl -u contextgo -f         # 查看日誌"
}

# ─── 建立桌面捷徑 ─────────────────────────────────────────────────────────────
create_desktop_entry() {
    local desktop_dir="${HOME}/.local/share/applications"
    local desktop_file="${desktop_dir}/contextgo.desktop"
...
print_summary() {
    ...
    echo -e "${GREEN}${BOLD}  🎉 ContextGo v${VERSION} 安裝完成！${NC}"
    echo -e "${GREEN}${BOLD}══════════════════════════════════════════════════${NC}"
    echo ""
    echo -e "  ${BOLD}📍 執行檔位置:${NC}  /usr/bin/ContextGo"
    echo -e "  ${BOLD}📍 管理腳本:${NC}    /opt/ContextGo/start-contextgo.sh"
    echo ""
    ...
    if [[ "${MODE}" == "headless" ]]; then
        ...
        echo ""
        echo "    # 使用管理腳本"
        echo "    /opt/ContextGo/start-contextgo.sh start"
        echo "    /opt/ContextGo/start-contextgo.sh status"
        echo "    /opt/ContextGo/start-contextgo.sh stop"
        echo ""
        if command -v systemctl &>/dev/null; then
            echo "    # 或使用 systemd"
            echo "    sudo systemctl start contextgo"
            echo "    sudo systemctl enable contextgo  # 開機自啟"
            echo ""
        fi
        ...
        echo ""
        echo "    # 直接啟動（桌面環境）"
        echo "    ContextGo --no-sandbox"
        echo ""
        echo "    # 或從應用程式選單尋找 ContextGo"
        echo ""
    fi

    echo -e "  ${BOLD}📖 文件:${NC}  https://github.com/contextgo/contextgo"
    echo -e "  ${BOLD}🐛 回報:${NC}  https://github.com/contextgo/contextgo/issues"
    echo ""

    if [[ "${MODE}" == "headless" ]]; then
        echo -e "  ${YELLOW}💡 提示:${NC}"
        echo "     • 設定工作目錄: export CONTEXTGO_WORKDIR=/path/to/workspace"
