...
const KILLABLE_NAMES = new Set(['electron', 'contextgo', 'contextgo.exe']);
...
function listLikelyConflictingProcesses() {
  try {
    if (isWindows()) {
      const output = run(
        "powershell -NoProfile -Command \"Get-Process | Where-Object { $_.ProcessName -in @('electron','ContextGo','node','bun') } | Select-Object ProcessName,Id | ConvertTo-Json -Compress\""
      );
      const parsed = output ? JSON.parse(output) : [];
      return Array.isArray(parsed) ? parsed : [parsed];
    }

    const output = run(`ps -A -o pid=,comm= | egrep "electron|ContextGo|node|bun" || true`);
    return output
      .split(/\r?\n/)
      .filter(Boolean)
      .map((line) => {
...
function cleanupByName() {
  ...
  for (const proc of processes) {
    ...
    if (!['electron', 'contextgo'].some((k) => rawName.includes(k))) continue;
    if (killPid(pid)) {
...
function launch(scriptName, withExtensions) {
  ...
  if (withExtensions) {
    env.CONTEXTGO_EXTENSIONS_PATH = path.resolve(process.cwd(), 'examples');
    log(`CONTEXTGO_EXTENSIONS_PATH=${env.CONTEXTGO_EXTENSIONS_PATH}`);
  }
