...
/**
 * Simplified build script for ContextGo
...
try {
  ...
  if (process.platform === 'win32' && builderArgs.includes('--win')) {
    ...
    if (!cleaned) {
      const aionRunning = isProcessRunningWindows('ContextGo.exe');
      const electronRunning = isProcessRunningWindows('electron.exe');
      if (aionRunning || electronRunning) {
        console.log('⚠️  Detected running ContextGo/Electron process. Attempting to close...');
        killWindowsProcesses(['ContextGo.exe', 'electron.exe']);
        cleaned = tryRemoveDir(winUnpackedDir);
        if (!cleaned) {
          console.log('⚠️  Directory still locked. Please close any running ContextGo/Electron processes and retry.');
        }
      }
    }
  }
  ...
  try {
    ...
    const winExePath = path.join(outDir, 'win-unpacked', 'ContextGo.exe');
    ...
    console.log('⚠️  Windows local build failed after ContextGo.exe was produced.');
    if (firstError) {
