# ContextGo WebUI Mode - Startup Guide

ContextGo supports WebUI mode, allowing you to access the application through a web browser. This guide covers how to start WebUI mode on all supported platforms.
...
WebUI mode starts ContextGo with an embedded web server, allowing you to:

- Access the application through any modern web browser
- Use ContextGo from remote devices on the same network (with `--remote` flag)
...
```cmd
# Using full path
"C:\Program Files\ContextGo\ContextGo.exe" --webui

# Or if ContextGo is in your PATH
ContextGo.exe --webui
```
...
```bash
# Using full path
/Applications/ContextGo.app/Contents/MacOS/ContextGo --webui

# Or using open command
open -a ContextGo --args --webui
```

### Method 2: Create Shell Script

Create `start-contextgo-webui.sh`:
...
```bash
chmod +x start-contextgo-webui.sh
./start-contextgo-webui.sh
```
...
chmod +x ContextGo-*.AppImage

# Run with --webui flag
./ContextGo-*.AppImage --webui
```

### Method 2: Create Desktop Entry

Create `~/.local/share/applications/contextgo-webui.desktop`:
...
```bash
chmod +x ~/.local/share/applications/contextgo-webui.desktop
```
...
Create `~/bin/start-contextgo-webui.sh`:
...
```bash
chmod +x ~/bin/start-contextgo-webui.sh
```
...
Create `/etc/systemd/system/contextgo-webui.service`:

```ini
[Unit]
Description=ContextGo WebUI Service
After=network.target

[Service]
Type=simple
User=YOUR_USERNAME
ExecStart=/opt/ContextGo/contextgo --webui --remote
Restart=on-failure
RestartSec=10
...
sudo systemctl enable contextgo-webui.service
sudo systemctl start contextgo-webui.service

# Check status
sudo systemctl status contextgo-webui.service
```
...
**Important Note**: Electron desktop mode is **not supported** on Android. However, you can run ContextGo in WebUI mode using Termux with a prooted Linux environment.

> **Community Contribution**: This guide is contributed by [@Manamama](https://github.com/Manamama). Special thanks for making ContextGo accessible on Android devices! 🙏
>
> **Original Tutorial**: [Running ContextGo WebUI on Android via Termux + Proot Ubuntu](https://gist.github.com/Manamama/b4f903c279b5e73bdad4c2c0a58d5ddd)
>
> **Related Issues**: [#217 - Android Support Discussion](https://github.com/contextgo/contextgo/issues/217)
...
#### 4. Download and Install ContextGo

```bash
# Download the ARM64 .deb package (replace VERSION with the actual version)
# Check latest version at: https://github.com/contextgo/contextgo/releases
wget https://github.com/contextgo/contextgo/releases/download/vVERSION/ContextGo_VERSION_arm64.deb

# Example (replace VERSION with the release tag, e.g. v1.5.2):
wget https://github.com/contextgo/contextgo/releases/download/vVERSION/ContextGo_VERSION_arm64.deb

# Install the package
apt install -y ./ContextGo_*.deb

# Verify installation
which ContextGo
```

#### 5. Launch ContextGo WebUI

```bash
# Start ContextGo in WebUI mode with no-sandbox flag
ContextGo --no-sandbox --webui
```
...
To access ContextGo from other devices on your local network:
...
ContextGo --no-sandbox --webui --port 8080
```
...
ContextGo requires sufficient RAM. Close other apps if you encounter memory issues.

#### Cannot Access from Browser

1. Check if ContextGo is running: look for "Server started" message
...
4. **Keep device charged** - Running ContextGo consumes battery

### Tested Environment

- **Device**: Android 14
- **Termux Version**: 0.118.0
- **ContextGo Version**: Latest release (e.g. 1.5.2)
- **Proot-distro**: Ubuntu (latest)

### Creating a Startup Script

For convenience, create a script to launch ContextGo quickly:
...
```bash
proot-distro login ubuntu -- bash -c "ContextGo --no-sandbox --webui --remote"
```
...
2. Report issues at [GitHub Issues #217](https://github.com/contextgo/contextgo/issues/217)
...
```cmd
ContextGo.exe --webui --remote
```

### macOS

```bash
/Applications/ContextGo.app/Contents/MacOS/ContextGo --webui --remote
```

### Linux

```bash
contextgo --webui --remote
```
...
netsh advfirewall firewall add rule name="ContextGo WebUI" dir=in action=allow protocol=TCP localport=3000
```
...
**macOS:**
Go to **System Preferences** → **Security & Privacy** → **Firewall** → **Firewall Options** → Add ContextGo
...
```powershell
& "C:\Program Files\ContextGo\ContextGo.exe" --webui 2>&1 | Tee-Object -FilePath contextgo.log
```

**macOS/Linux:**

```bash
/path/to/contextgo --webui 2>&1 | tee contextgo.log
```
...
| Platform | Location                                                    |
| -------- | ----------------------------------------------------------- |
| Windows  | `%APPDATA%/ContextGo/webui.config.json`                     |
| macOS    | `~/Library/Application Support/ContextGo/webui.config.json` |
| Linux    | `~/.config/ContextGo/webui.config.json`                     |
...
```cmd
# Using full path
"C:\Program Files\ContextGo\ContextGo.exe" --resetpass

# Or for a specific user
"C:\Program Files\ContextGo\ContextGo.exe" --resetpass username
```
...
```bash
# Using full path
/Applications/ContextGo.app/Contents/MacOS/ContextGo --resetpass

# Or for a specific user
/Applications/ContextGo.app/Contents/MacOS/ContextGo --resetpass username
```
...
# Or for a specific user
contextgo --resetpass username

# Or using full path
/opt/ContextGo/contextgo --resetpass
```
...
2. Search [existing issues](https://github.com/contextgo/contextgo/issues)
3. Create a [new issue](https://github.com/contextgo/contextgo/issues/new) with:
   - Your OS and version
   - ContextGo version
   - Steps to reproduce
...
**Happy using ContextGo in WebUI mode!** 🚀
