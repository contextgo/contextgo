...
ContextGo supports CDP for external debugging tools integration. In development mode (`just dev`), CDP is enabled by default on port 9230.

## Enable CDP in Production

1. Open ContextGo Settings → System → Developer Debug
...
ContextGo can integrate with other MCP tools for enhanced development experience:
...
- `list_pages` — List all open pages in ContextGo
...
ContextGo 支持 CDP 用于外部调试工具集成。在开发模式 (`just dev`) 下，CDP 默认在端口 9230 启用。

## 在生产环境启用 CDP

1. 打开 ContextGo 设置 → 系统 → 开发者调试
2. 启用"启用远程调试 (CDP)"
3. 重启应用
...
ContextGo 可以集成其他 MCP 工具来增强开发体验：
...
配置完成后，可以使用 MCP 工具与 ContextGo 交互：

- `list_pages` — 列出 ContextGo 中所有打开的页面
