...
export type ManagedRuntimeModelProvider = 'infermesh';

export type ManagedRuntimeTokenGroup = {
  name: string;
  displayName: string;
  description?: string;
};

export type ManagedRuntimeTokenGroupListParams = {
  provider: ManagedRuntimeModelProvider;
};

export type ManagedRuntimeTokenGroupListResult = {
  provider: ManagedRuntimeModelProvider;
  groups: ManagedRuntimeTokenGroup[];
};

export type ManagedRuntimeModelProviderSyncParams = {
  provider: ManagedRuntimeModelProvider;
  group?: string;
};

export type ManagedRuntimeModelProviderSyncResult = {
  provider: ManagedRuntimeModelProvider;
  tokenGroup?: string;
  modelCount: number;
};

export type ManagedRuntimeModelConfigParams = {
  backend: AcpBackend;
  provider: ManagedRuntimeModelProvider;
  model?: string;
  group?: string;
...
export type ManagedRuntimeModelConfigResult = {
  backend: AcpBackend;
  provider: ManagedRuntimeModelProvider;
  model: string;
  baseUrl: string;
  writtenEntries: ManagedRuntimeConfigEntry[];
};
...
export type LocalTokenUsageRuntime = Extract<AcpBackend, 'claude' | 'codex' | 'gemini' | 'opencode'>;

export type LocalTokenUsageStatus = 'ok' | 'empty' | 'unsupported' | 'error';

export type LocalTokenUsageTotals = {
  inputTokens: number;
  outputTokens: number;
...
export type LocalTokenUsageDailyReport = {
  date: string;
  totals: LocalTokenUsageTotals;
  modelsUsed: string[];
};

export type LocalTokenUsageRuntimeReport = {
  backend: LocalTokenUsageRuntime;
  label: string;
  status: LocalTokenUsageStatus;
  source: 'ccusage' | 'ccusage-codex' | 'ccusage-opencode' | 'unsupported';
  sourcePath?: string;
  command?: string;
  error?: string;
  updatedAt: string;
  totals: LocalTokenUsageTotals;
  today: LocalTokenUsageTotals;
  days: LocalTokenUsageDailyReport[];
};

export type LocalTokenUsageReport = {
  generatedAt: string;
  totals: LocalTokenUsageTotals;
  today: LocalTokenUsageTotals;
  runtimes: LocalTokenUsageRuntimeReport[];
};
