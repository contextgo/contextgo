  ...
  useEffect(() => {
    ...
    const handleConsoleMessage = (event: Electron.ConsoleMessageEvent) => {
      try {
        ...
        if (event.message.includes('__CONTEXTGO_WEBVIEW_ZOOM__')) {
        ...
        if (event.message.includes('__CONTEXTGO_WEBVIEW_ZOOM_RESET__')) {
          setZoomFactor(1);
        }
      } catch {
        // Ignore parse errors
        ...
        webviewEl
          ...
          (function() {
            ...
            window.addEventListener('wheel', function(e) {
              ...
              e.preventDefault();
              console.log('__CONTEXTGO_WEBVIEW_ZOOM__', JSON.stringify({ deltaY: e.deltaY }));
            }, { passive: false, capture: true });
            window.addEventListener('keydown', function(e) {
              if (!(e.ctrlKey || e.metaKey)) return;
              if (e.key === '0') {
                e.preventDefault();
                console.log('__CONTEXTGO_WEBVIEW_ZOOM_RESET__');
              }
            }, { capture: true });
            return true;
          })();
