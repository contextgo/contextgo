...
export default function AffineCanvasSurface(props: AffineCanvasSurfaceProps) {
  ...
  const activeBoard = useMemo(() => props.boards.find((board) => board.id === activeBoardId), [activeBoardId, props.boards]);
  ...
  const renderActiveBoardPreview = () => {
    return (
      <Card size='small' title={`Active Board Cards (${activeBoardCards.length})${activeBoard ? ` · ${activeBoard.title}` : ''}`}>
        {!activeBoard ? (
