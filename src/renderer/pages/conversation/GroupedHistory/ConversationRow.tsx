  ...
  const renderLeadingIcon = () => {
    ...
    if (assistantInfo) {
      if (assistantInfo.isEmoji) {
        return (
          <span className='inline-flex h-20px w-20px items-center justify-center text-18px leading-none flex-shrink-0'>
            {assistantInfo.logo}
          </span>
        );
      }
      return (
        <span className='inline-flex h-20px w-20px items-center justify-center flex-shrink-0 leading-none'>
          <img
            src={assistantInfo.logo}
            alt={assistantInfo.name}
            className='block w-20px h-20px rounded-50% object-contain flex-shrink-0'
          />
        </span>
      );
    }
