...
export const resolveConversationTabDensity = ({
  ...
  if (showHeaderActions && containerWidth > 0) {
...
export const resolveConversationTabWidth = ({
  ...
  if (density === 'compact') {
    return clamp(widthPerTab - 8, 76, 96);
  }

  return clamp(widthPerTab - 10, 118, 184);
};
...
const ConversationTabView: React.FC<ConversationTabViewProps> = ({
  ...
  const showCloseButton = !isIconOnly && (density === 'full' || isActive);
  ...
  } ${isIconOnly ? 'justify-center px-0' : isCompact ? 'gap-7px px-8px' : 'gap-8px px-10px'}`;
...
const ConversationTabs: React.FC<{ showHeaderActions?: boolean }> = ({ showHeaderActions = true }) => {
  ...
  const tabsRootClassName = isMobile
    ? 'relative shrink-0 bg-1 min-h-42px px-8px py-4px'
    : 'relative flex h-full shrink-0 items-center bg-1 px-8px';
  const tabsInnerClassName = isMobile
    ? 'relative flex items-center h-32px w-full gap-8px'
    : 'relative flex h-full w-full items-center gap-8px';
    ...
    <div className={tabsRootClassName}>
      <div className={tabsInnerClassName}>
        ...
        <div
          ...
          className='flex items-center h-full min-w-0 flex-1 gap-6px overflow-x-auto overflow-y-hidden [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden'
        >
          {openTabs.map((tab) => (
            <ConversationTabView
              key={tab.id}
