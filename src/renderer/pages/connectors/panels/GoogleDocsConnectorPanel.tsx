...
const GoogleDocsConnectorPanel: React.FC<GoogleDocsConnectorPanelProps> = ({ connectorId }) => {
  ...
  useEffect(() => {
    if (!isGoogleDocsConnector) return;
    const unsub = ipcBridge.googleDocsConnector.statusChanged.on?.((nextStatus: GoogleDocsConnectorRuntimeStatus) =>
      ...
      <div className={styles.detailGrid}>
        <div className={styles.detailCard}>
          ...
          <Spin loading={loading} block>
            ...
            <div className={styles.clipboardInfoList}>
              ...
              <div>
                <strong>Docs stored:</strong> {status?.docCount ?? 0}
              </div>
              <div>
          ...
          <Spin loading={loading} block>
            <div className={styles.clipboardControlList}>
              ...
              <div className={styles.clipboardControlRowColumn}>
                <span>Scopes</span>
                <Input
                  value={config?.scopes.join(', ') ?? DEFAULT_SCOPES}
                  onChange={(value) =>
                    patchConfig({
                      scopes: value
                        .split(',')
            ...
            <div className={styles.clipboardActionRow}>
              <Button type='primary' onClick={() => void saveConfig()} loading={saving}>
                Save Google Docs Config
              </Button>
            </div>
            <div className={styles.clipboardNote}>
              Google Docs reuses the same Google Workspace OAuth token cache as Google Drive.
            </div>
          </Spin>
        </div>
        ...
        <div className={styles.detailCard}>
          ...
          <div className={styles.clipboardList} data-testid='google-docs-stored-list'>
            {storedDocs.length === 0 ? (
              <div className={styles.detailCardText}>No persisted Google Docs in ContextGo store yet.</div>
            ) : null}
            {storedDocs.map((doc) => (
