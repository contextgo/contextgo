...
interface CronPresetLibraryProps {
  presets: CronPreset[];
  previewOnly?: boolean;
  creatingPresetId?: CronPresetId | null;
  helperText?: string;
  onCreatePreset?: (preset: CronPreset) => void;
  onFillPreset?: (preset: CronPreset) => void;
}

const CronPresetLibrary: React.FC<CronPresetLibraryProps> = ({
  presets,
  previewOnly = false,
      ...
      <div className='flex flex-col gap-8px'>
          ...
          <div className='flex flex-col gap-10px'>
            <div className='flex items-start justify-between gap-10px flex-wrap'>
                  ...
                  <Typography.Text className='text-13px font-semibold text-t-primary'>
                    {t(
                      activePack === 'all'
                        ...
                        : `cron.presets.packs.${activePackDetailKey}`
                    )}
                  </Typography.Text>
                  {isRecommendedPack ? (
                    <Tag size='small' color='green'>
                      {t('cron.presets.recommended')}
                    </Tag>
                  ) : null}
                </div>
                <Typography.Text className='mt-4px block text-12px leading-6 text-t-secondary'>
                  {t(`cron.presets.packDetails.${activePackDetailKey}.description`)}
                </Typography.Text>
              </div>
              <span className='inline-flex items-center rounded-full bg-[var(--color-fill-2)] px-8px py-2px text-11px font-semibold text-t-tertiary'>
                {activePackPresetCount} {t('cron.presets.packDetails.presetCount')}
              </span>
            </div>
            ...
            <div className='grid grid-cols-1 gap-8px md:grid-cols-2'>
              <div className='bg-[var(--bg-1)] px-10px py-8px rd-12px'>
                <Typography.Text className='text-11px font-medium text-t-tertiary'>
                  {t('cron.presets.packDetails.bestForLabel')}
                </Typography.Text>
                <Typography.Text className='mt-4px block text-12px leading-6 text-t-secondary'>
                  {t(`cron.presets.packDetails.${activePackDetailKey}.bestFor`)}
                </Typography.Text>
              </div>
              <div className='bg-[var(--bg-1)] px-10px py-8px rd-12px'>
                <Typography.Text className='text-11px font-medium text-t-tertiary'>
                  {t('cron.presets.packDetails.outcomeLabel')}
                </Typography.Text>
                <Typography.Text className='mt-4px block text-12px leading-6 text-t-secondary'>
                  {t(`cron.presets.packDetails.${activePackDetailKey}.outcome`)}
                </Typography.Text>
              </div>
            </div>
          </div>
        ...
        <div className='flex flex-wrap gap-8px'>
          {packOptions.map((option) => (
            ...
            >
              {t(`cron.presets.packs.${option.value}`)} {option.count}
            </Button>
          ))}
          <Button size='mini' type={activePack === 'all' ? 'outline' : 'text'} onClick={() => handlePackChange('all')}>
            {t('cron.presets.actions.browseAllPacks')}
          </Button>
        </div>
      </div>
          ...
          <div key={group.category} className='flex flex-col gap-10px'>
            <div className='flex items-center justify-between gap-10px'>
              <Typography.Text className='text-12px font-semibold tracking-[0.04em] text-t-secondary uppercase'>
                {t(`cron.presets.categories.${group.category}`)}
              </Typography.Text>
                    ...
                    <div className='min-w-0 flex-1'>
                      <div className='flex items-center gap-6px flex-wrap'>
                        ...
                        <Tag size='small' color={categoryColorMap[preset.category]}>
                          {t(`cron.presets.categories.${preset.category}`)}
                        </Tag>
                  ...
                  {!previewOnly && (onCreatePreset || onFillPreset) && (
                    <div className='mt-12px flex flex-wrap gap-8px'>
                      {onCreatePreset && (
                        ...
                        >
                          {t('cron.presets.actions.useNow')}
                        </Button>
                      )}
                      {onFillPreset && (
                        <Button size='mini' onClick={() => onFillPreset(preset)}>
                          {t('cron.presets.actions.fillPrompt')}
                        </Button>
                      )}
                    </div>
                  )}
...
export default CronPresetLibrary;
