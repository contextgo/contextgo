...
import { getPptPreviewCacheDir } from '@process/services/conversionService';
...
export function registerApiRoutes(app: Express): void {
  ...
  /**
   * 预览缓存文件 API - Preview cache file API
   * GET /api/preview-file?path={absolutePath}
   *
   * Only files generated into the dedicated PPT preview cache directory are allowed.
   * This keeps WebUI PPT preview available without exposing arbitrary local paths.
   */
  app.get('/api/preview-file', apiRateLimiter, validateApiAccess, (req: Request, res: Response) => {
    const rawPath = typeof req.query.path === 'string' ? req.query.path : '';
    if (!rawPath) {
      return res.status(400).json({ message: 'Missing path query parameter' });
    }
    ...
    const normalizedPath = path.resolve(rawPath);
    const previewRoot = path.resolve(getPptPreviewCacheDir());
    const isInsidePreviewRoot =
      normalizedPath === previewRoot || normalizedPath.startsWith(`${previewRoot}${path.sep}`);

    if (!isInsidePreviewRoot) {
      return res.status(403).json({
        message: 'Access denied: path is outside preview cache directory',
      });
    }

    if (path.extname(normalizedPath).toLowerCase() !== '.pdf') {
      return res.status(403).json({
        message: 'Access denied: only PDF preview files are allowed',
      });
    }
