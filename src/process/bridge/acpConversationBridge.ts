...
import { getInfermeshRuntimeProvisioningService } from '@process/services/cloud/InfermeshRuntimeProvisioningService';
import { getLocalTokenUsageService } from '@process/services/usage';
import {
  ACP_BACKENDS_ALL,
...
function resolveManagedRuntimeConfigEntries(backend: AcpBackend): ManagedRuntimeConfigEntry[] {
  switch (backend) {
    case 'gemini': {
      ...
      return [
        { kind: 'config', path: USER_SETTINGS_PATH, exists: fs.existsSync(USER_SETTINGS_PATH) },
        { kind: 'auth', path: geminiEnvPath, exists: fs.existsSync(geminiEnvPath) },
      ];
    }
    case 'claude':
      return [
...
export function initAcpConversationBridge(
  ...
  getLocalTokenUsageService().startBackgroundRefresh();
  ...
  ipcBridge.acpConversation.configureManagedRuntimeModel.provider(async (params) => {
    try {
      const result = await getInfermeshRuntimeProvisioningService().configure(params);
      return {
        success: true,
        data: result,
      };
  ...
  ipcBridge.acpConversation.listManagedRuntimeTokenGroups.provider(async (params) => {
    try {
      const result = await getInfermeshRuntimeProvisioningService().listTokenGroups(params);
      return {
        success: true,
        data: result,
      };
  ...
  ipcBridge.acpConversation.syncManagedRuntimeModelProvider.provider(async (params) => {
    try {
      const result = await getInfermeshRuntimeProvisioningService().syncProvider(params);
      return {
        success: true,
        data: result,
      };
  ...
  ipcBridge.acpConversation.getLocalTokenUsage.provider(async (params) => {
    try {
      return {
        success: true,
        data: await getLocalTokenUsageService().getReport({ forceRefresh: params?.forceRefresh === true }),
      };
    } catch (error) {
      return {
        success: false,
