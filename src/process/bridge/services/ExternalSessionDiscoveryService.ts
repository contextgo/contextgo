...
import { planExternalSessionWorkspaceBootstrap } from '@/process/bridge/services/externalSessionBootstrap';
...
export class ExternalSessionDiscoveryService {
  ...
  private async importClaudeSession(sessionId: string): Promise<TChatConversation> {
    ...
    const bootstrapPlan = await planExternalSessionWorkspaceBootstrap('claude', session.workspace);
    const conversation = await this.conversationService.createConversation({
      ...
      extra: {
        ...
        nativeWorkspaceBootstrap: bootstrapPlan.nativeWorkspaceBootstrap,
        presetAssistantId: bootstrapPlan.presetAssistantId,
        enabledSkills: bootstrapPlan.enabledSkills,
        enabledHooks: bootstrapPlan.enabledHooks,
        externalWorkspaceInspection: {
          hasContextgoDir: bootstrapPlan.inspection.hasContextgoDir,
          hasAgentsMd: bootstrapPlan.inspection.hasAgentsMd,
          capabilityCounts: { ...bootstrapPlan.inspection.capabilityCounts },
          hasProjectCapabilitySurface: bootstrapPlan.inspection.hasProjectCapabilitySurface,
          hasProjectContextSurface: bootstrapPlan.inspection.hasProjectContextSurface,
        },
      },
    });
  ...
  private async importCodexSession(sessionId: string): Promise<TChatConversation> {
    ...
    const bootstrapPlan = await planExternalSessionWorkspaceBootstrap('codex', session.workspace);
    const conversation = await this.conversationService.createConversation({
      type: 'acp',
      name: session.title || path.basename(session.workspace) || session.sessionId,
      model: {
  ...
  private async importGeminiSession(sessionId: string): Promise<TChatConversation> {
    ...
    const bootstrapPlan = await planExternalSessionWorkspaceBootstrap('gemini', session.workspace);
    const conversation = await this.conversationService.createConversation({
      type: 'acp',
      name: session.title || path.basename(session.workspace) || session.sessionId,
      model: {
  ...
  private async importOpencodeSession(sessionId: string): Promise<TChatConversation> {
    ...
    const bootstrapPlan = await planExternalSessionWorkspaceBootstrap('opencode', session.workspace);
    const conversation = await this.conversationService.createConversation({
      type: 'acp',
      name: session.title || path.basename(session.workspace) || session.sessionId,
      model: {
