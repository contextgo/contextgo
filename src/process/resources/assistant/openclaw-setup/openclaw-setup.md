...
**Option B: Start ContextGo WebUI Remote Mode**

- **Experience**: Access through browser with ContextGo's full interface features
...
- **Configuration requirements**: Start ContextGo WebUI service, access through browser
...
5. **Post-installation configuration guidance** (IMPORTANT):
   ...
   - **Usage guidance**:
     - **Local usage**: "After OpenClaw installation is complete, **please restart ContextGo**, then you can see OpenClaw in the available Agent list on the ContextGo homepage and start chatting directly."
...
1. **Ask user preference first**: Ask user which method they want to configure → **Wait for user reply**
   - "Do you want to connect directly to IM channels (like Telegram, WhatsApp, etc.), or use ContextGo WebUI remote mode?"
...
4. **Option B: Start ContextGo WebUI Remote Mode**
   - **Must use contextgo-webui-setup skill**: Consult `references/contextgo-webui.md`
   - **Workflow**:
     1. Ask user needs: Same WiFi, cross-network access, or server deployment? → **Wait for user reply**
     2. After user replies, **guide user to ContextGo settings interface**:
        - **Open settings interface**: Clearly tell user how to open it
          - "Please click the **Settings icon** (gear icon) at the bottom left of ContextGo"
          - "In the settings menu, click the **'WebUI'** option"
          - "Enter the WebUI configuration interface"
        - **Configuration steps**: Follow `contextgo-webui-setup` skill's `references/contextgo-webui.md` documentation to guide user:
   ...
   - **Key principles**:
     ...
     - **Do not attempt to install a separate WebUI npm package or any similar package**: WebUI is a built-in feature of ContextGo, not a separate package
...
### contextgo-webui-setup Skill

**Core documentation**: `references/contextgo-webui.md`

**When to use**: When user chooses WebUI option, use immediately

**How to use**:

1. **Directly consult `references/contextgo-webui.md`** and guide user to complete configuration following the documentation
...
5. **Post-installation configuration guidance**:
   ...
   - **Usage guidance**:
     - Introduce local usage (return to ContextGo homepage)
...
3. **Choose WebUI**: Use `contextgo-webui-setup` skill → Ask needs → Choose solution → Execute configuration → Provide usage instructions
...
7. **Skill usage**:
   ...
   - WebUI configuration → **Must use `contextgo-webui-setup` skill** (directly consult `references/contextgo-webui.md` and follow documentation, don't repeat detailed steps from documentation)
