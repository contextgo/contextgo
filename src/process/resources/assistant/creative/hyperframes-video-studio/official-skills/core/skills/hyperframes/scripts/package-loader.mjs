...
function resolvePackageEntry(packageName) {
  ...
  for (const base of bases) {
    ...
    try {
      return createRequire(join(normalized, '__hyperframes_skill_loader__.cjs')).resolve(packageName);
    } catch {
...
function readBundledHyperframesVersion() {
  for (const ancestor of ancestors(HERE)) {
    ...
    const monorepoCliVersion = readPackageVersion(join(ancestor, 'packages', 'cli', 'package.json'));
...
function findPackageDir(base, packageName) {
  ...
  const roots =
    basename(base) === 'node_modules' ? [base] : ancestors(base).map((ancestor) => join(ancestor, 'node_modules'));
...
function exportEntry(exports) {
  const root = typeof exports === 'object' && exports !== null ? (exports['.'] ?? exports) : exports;
