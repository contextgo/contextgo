...
export const normalizeGmailConnectorConfig = (input?: Partial<GmailConnectorConfig> | null): GmailConnectorConfig => ({ enabled: input?.enabled ?? false, clientId: input?.clientId?.trim() ?? '', clientSecret: input?.clientSecret?.trim() ?? '', scopes: normalizeScopes(input?.scopes), command: input?.command?.trim() || 'go', args: Array.isArray(input?.args) && input.args.length ? [...input.args] : ['run', '.'] });
export class GmailConnectorService {
  private state: GmailConnectorRuntimeStatus = { lifecycle: 'stopped', desiredState: 'stopped', available: true, note: 'Gmail connector reuses Google Workspace OAuth and a managed Go sidecar.', hasCredentials: false };
  ...
  async getConfig(): Promise<GmailConnectorConfig> { const stored = await (await this.getStore()).get(KEY); return normalizeGmailConnectorConfig(stored ?? undefined); }
  async setConfig(updates: Partial<GmailConnectorConfig>): Promise<GmailConnectorConfig> { const next = normalizeGmailConnectorConfig({ ...(await this.getConfig()), ...updates }); await (await this.getStore()).set(KEY, next); return next; }
  async getStatus(): Promise<GmailConnectorRuntimeStatus> { const config = await this.getConfig(); const details = this.controller.getRuntimeDetails(); const tokenCachePath = path.join(await resolveGoogleWorkspaceOAuthDir(), 'google-drive-token.json'); const tokenMeta = existsSync(tokenCachePath) ? await this.readTokenMeta(tokenCachePath) : null; return { ...this.state, lifecycle: details.running ? 'running' : this.state.desiredState === 'running' ? 'error' : this.state.lifecycle, hasCredentials: Boolean(config.clientId && config.clientSecret), command: details.command, args: details.args, pid: details.pid, tokenCachePath, hasCachedToken: existsSync(tokenCachePath), hasRefreshToken: Boolean(tokenMeta?.refresh_token), tokenExpiry: tokenMeta?.expiry, tokenScope: tokenMeta?.scope, lastError: details.lastError ?? this.state.lastError, note: details.running ? 'Managed Gmail sidecar is running.' : this.state.note }; }
  async start(): Promise<GmailConnectorRuntimeStatus> { const config = await this.getConfig(); const now = Date.now(); if (!config.clientId || !config.clientSecret) { this.state = { ...this.state, lifecycle: 'error', desiredState: 'running', hasCredentials: false, note: 'Gmail connector requires Client ID and Client Secret before launch.', lastError: 'Gmail connector requires Client ID and Client Secret before launch.', lastStartAt: now }; return this.getStatus(); } const result = await this.controller.start(config); this.state = { ...this.state, lifecycle: 'running', desiredState: 'running', hasCredentials: true, command: result.command, args: result.args, pid: result.pid, note: result.note, lastStartAt: now, lastError: undefined }; return this.getStatus(); }
  async stop(): Promise<GmailConnectorRuntimeStatus> { await this.controller.stop(); this.state = { ...this.state, lifecycle: 'stopped', desiredState: 'stopped', pid: undefined, note: 'Managed Gmail sidecar is stopped.', lastStopAt: Date.now(), lastError: undefined }; return this.getStatus(); }
