...
export const normalizeGoogleCalendarConnectorConfig = (
  input?: Partial<GoogleCalendarConnectorConfig> | null
): GoogleCalendarConnectorConfig => ({
...
export class GoogleCalendarConnectorService {
  private state: GoogleCalendarConnectorRuntimeStatus = {
    lifecycle: 'stopped',
    desiredState: 'stopped',
    available: true,
    note: 'Google Calendar connector reuses Google Workspace OAuth and a managed Go sidecar.',
  ...
  async getConfig(): Promise<GoogleCalendarConnectorConfig> {
    const stored = await (await this.getStore()).get(KEY);
    return normalizeGoogleCalendarConnectorConfig(stored ?? undefined);
  }
  async setConfig(updates: Partial<GoogleCalendarConnectorConfig>): Promise<GoogleCalendarConnectorConfig> {
    const next = normalizeGoogleCalendarConnectorConfig({ ...(await this.getConfig()), ...updates });
  ...
  async getStatus(): Promise<GoogleCalendarConnectorRuntimeStatus> {
    ...
    return {
      ...
      note: details.running ? 'Managed Google Calendar sidecar is running.' : this.state.note,
    };
  }
  async start(): Promise<GoogleCalendarConnectorRuntimeStatus> {
    ...
    if (!config.clientId || !config.clientSecret) {
      this.state = {
        ...
        hasCredentials: false,
        note: 'Google Calendar connector requires Client ID and Client Secret before launch.',
        lastError: 'Google Calendar connector requires Client ID and Client Secret before launch.',
        lastStartAt: now,
      };
  ...
  async stop(): Promise<GoogleCalendarConnectorRuntimeStatus> {
    ...
    this.state = {
      ...
      desiredState: 'stopped',
      pid: undefined,
      note: 'Managed Google Calendar sidecar is stopped.',
      lastStopAt: Date.now(),
      lastError: undefined,
    };
    return this.getStatus();
  }
  async listCalendars(): Promise<readonly GoogleCalendarEntry[]> {
    const payload = JSON.parse(await this.executeSidecar(['--list-calendars-json'])) as {
