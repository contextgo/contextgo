...
const KEY = 'connector.googleSheets.config';
const DEFAULT_SCOPES = [
  'https://www.googleapis.com/auth/spreadsheets.readonly',
  'https://www.googleapis.com/auth/drive.metadata.readonly',
];
type StoreRef = Pick<IConfigStorageRefer, 'connector.googleSheets.config'>;
type Store = {
...
export const normalizeGoogleSheetsConnectorConfig = (
  input?: Partial<GoogleSheetsConnectorConfig> | null
): GoogleSheetsConnectorConfig => ({
...
export class GoogleSheetsConnectorService {
  private state: GoogleSheetsConnectorRuntimeStatus = {
    lifecycle: 'stopped',
    desiredState: 'stopped',
    available: true,
    note: 'Google Sheets connector reuses Google Workspace OAuth and a managed Go sidecar.',
  ...
  async getConfig(): Promise<GoogleSheetsConnectorConfig> {
    const stored = await (await this.getStore()).get(KEY);
    return normalizeGoogleSheetsConnectorConfig(stored ?? undefined);
  }
  async setConfig(updates: Partial<GoogleSheetsConnectorConfig>): Promise<GoogleSheetsConnectorConfig> {
    const next = normalizeGoogleSheetsConnectorConfig({ ...(await this.getConfig()), ...updates });
    await (await this.getStore()).set(KEY, next);
    return next;
  }
  async getStatus(): Promise<GoogleSheetsConnectorRuntimeStatus> {
    const config = await this.getConfig();
    ...
    return {
      ...
      note: details.running ? 'Managed Google Sheets sidecar is running.' : this.state.note,
    };
  }
  async start(): Promise<GoogleSheetsConnectorRuntimeStatus> {
    const config = await this.getConfig();
    const now = Date.now();
    if (!config.clientId || !config.clientSecret) {
      this.state = {
        ...
        hasCredentials: false,
        note: 'Google Sheets connector requires Client ID and Client Secret before launch.',
        lastError: 'Google Sheets connector requires Client ID and Client Secret before launch.',
        lastStartAt: now,
      };
  ...
  async stop(): Promise<GoogleSheetsConnectorRuntimeStatus> {
    ...
    this.state = {
      ...
      desiredState: 'stopped',
      pid: undefined,
      note: 'Managed Google Sheets sidecar is stopped.',
      lastStopAt: Date.now(),
      lastError: undefined,
    };
    return this.getStatus();
  }
  async listSpreadsheets(limit = 20): Promise<readonly GoogleSheet[]> {
    const payload = JSON.parse(
      await this.executeSidecar(['--list-sheets-json', '--limit', String(Math.max(1, limit))])
    ...
    return (Array.isArray(payload.files) ? payload.files : []).map((file) => ({
      ...
      ownerNames: Array.isArray(file.ownerNames) ? file.ownerNames.map((v) => String(v)) : undefined,
