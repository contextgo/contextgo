...
export class AionUIDatabase {
  ...
  getContextSource(sourceId: string): IQueryResult<SourceRecord | null> {
    try {
      const row = this.db.prepare('SELECT * FROM context_sources WHERE id = ?').get(sourceId) as IContextSourceRow | undefined;
  ...
  getContextDocument(documentId: string): IQueryResult<DocumentSnapshot | null> {
    try {
      const row = this.db.prepare('SELECT * FROM context_documents WHERE id = ?').get(documentId) as IContextDocumentRow | undefined;
  ...
  getContextMemory(memoryId: string): IQueryResult<MemoryEntry | null> {
    try {
      const row = this.db.prepare('SELECT * FROM context_memories WHERE id = ?').get(memoryId) as IContextMemoryRow | undefined;
  ...
  saveContextMemory(memory: MemoryEntry): IQueryResult<MemoryEntry> {
    try {
      ...
      this.db
        ...
        `)
        .run(
          row.id, row.space_id, row.kind, row.summary, row.detail, row.source_ids, row.chunk_ids, row.confidence, row.tier, row.priority,
          row.state, row.superseded_by_id, row.expires_at, row.last_accessed_at, row.last_confirmed_at, row.created_at, row.updated_at
        );
  ...
  getContextMemoryCandidate(candidateId: string): IQueryResult<MemoryCandidateEntry | null> {
    try {
      const row = this.db
        .prepare('SELECT * FROM context_memory_candidates WHERE id = ?')
        .get(candidateId) as IContextMemoryCandidateRow | undefined;
  ...
  saveContextMemoryCandidate(candidate: MemoryCandidateEntry): IQueryResult<MemoryCandidateEntry> {
    try {
      ...
      this.db
        ...
        `)
        .run(
          row.id, row.space_id, row.thread_id, row.kind, row.tier, row.summary, row.detail, row.source_ids, row.chunk_ids,
          row.confidence, row.priority, row.evidence_count, row.repeated_across_sources, row.recent_reference_count,
          row.user_confirmed, row.manually_pinned, row.execution_backed, row.contradiction_detected, row.promotion_score,
          row.promotion_rationale, row.destination, row.state, row.review_status, row.promoted_memory_id, row.reviewed_at,
          row.reviewed_by, row.created_at, row.updated_at
        );
  ...
  getContextProfile(profileId: string): IQueryResult<ProfileSegment | null> {
    try {
      const row = this.db.prepare('SELECT * FROM context_profiles WHERE id = ?').get(profileId) as IContextProfileRow | undefined;
      ...
      this.db
        .prepare(`
          ...
          ON CONFLICT(id) DO UPDATE SET
            ...
            updated_at = excluded.updated_at
        `)
        .run(row.id, row.space_id, row.key, row.summary, row.memory_ids, row.confidence, row.state, row.created_at, row.updated_at);
  ...
  listContextOperations(spaceId: string, cursor?: { operationId: string; createdAt: string }): IQueryResult<ContextOperation[]> {
    try {
      const rows = (cursor
        ? this.db
            .prepare(
        ...
        : this.db
            ...
            .all(spaceId)) as IContextOperationRow[];
  ...
  getLatestContextOperationCursor(spaceId: string): IQueryResult<{ operationId: string; createdAt: string } | null> {
    try {
      const row = this.db
        .prepare('SELECT id, created_at FROM context_operations WHERE space_id = ? ORDER BY created_at DESC, id DESC LIMIT 1')
