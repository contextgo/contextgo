# ContextGo Database System

本文档介绍 ContextGo 的数据库系统，它使用 **better-sqlite3** (主进程) 作为持久化存储。
...
- **数据库文件**: `{dataPath}/contextgo.db`
- **图片文件**: `{dataPath}/images/`

其中 `{dataPath}` 为：

- macOS: `~/Library/Application Support/ContextGo/contextgo/`
- Windows: `%APPDATA%/ContextGo/contextgo/`
- Linux: `~/.config/ContextGo/contextgo/`
...
直接复制 `contextgo.db` 和 `contextgo.db-wal` 文件即可。
